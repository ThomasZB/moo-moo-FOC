...
void foc_calibrate_phase(void) {
  ...
  for (unsigned char counter = 0; counter < 50; ++counter) {
    ...
    if (current_angle > last_angle) positive_counter++;
...
/*!
 *   \brief      calculate the corresponding three-phase PWM duty cycle under
   the current electrical angle
 *  \param[in]  elect_angle: electrical angle of motor
   rotor, in radian system
 *  \param[in]  d: expected field strength in the direct
   axis direction of the motor
 * \param[in]  q: expected field strength in the
   quadrature axis direction of the motor
 * \param[out] u: calculation results of
   U-phase duty cycle
 * \param[out] v: calculation results of V-phase duty cycle
 * \param[out] w: calculation results of W-phase duty cycle
*/
void foc_calculate_dutycycle(float elect_angle, float d, float q, float *u,
                             float *v, float *w) {
  float alpha, beta;
  ...
  // *w = 1.5f - *u - *v;
  *u = 0.5 + alpha;
  *v = 0.5 + (-alpha * 0.5f + beta * 0.866025404f);
  *w = 0.5 + (-alpha * 0.5f - beta * 0.866025404f);
#else
